...
serve(async (req) => {
  ...
  try {
    ...
    // Check it's paid to FINTECH VENTURES LCC
    if (!msg.includes("FINTECH VENTURES LCC") && !msg.includes("FINTECH VENTURES")) {
      throw new Error("Payment must be made to FINTECH VENTURES LCC (Till 7172200). Please check and try again.");
    }
